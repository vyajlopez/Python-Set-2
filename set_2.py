...
def shift_by_letter(letter, letter_shift):
    ...
    new_letter = ((ord(letter) - ord('A') + shift_value) % 26) + ord('A')
    return chr(new_letter)
...
extended_key = (key * (len(message) // len(key) + 1))[:len(message)]
    ...
    for i in range(len(message)):
        ...
        if msg_char == ' ':
            ...
            original_pos = ord(msg_char) - ord('A')
...
def scytale_cipher(message, shift):
    ...
    for i in range(len(padded_message)):
        original_index = (i // shift) + cols * (i % shift)
        encrypted.append(padded_message[original_index])
    ...
    for k in range(len(message)):
        row = k // cols
        column = k % cols
        encrypted_index = column * shift + row
        decrypted.append(message[encrypted_index])
